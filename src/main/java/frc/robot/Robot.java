...
public class Robot extends TimedRobot {
  ...
  public static final Faults fault = new Faults();
  ...
  public void robotPeriodic() {
    ...
    SmartDashboard.putNumber("left side", driveSubsystem.countsToInches(driveSubsystem.getTicks()));
    SmartDashboard.putNumber("right side", driveSubsystem.countsToInches(driveSubsystem.getRightTicks()));

    SmartDashboard.putBoolean("ultrasonic range", sensorSubsystem.checkRange());  //siona wants this to output a color instead of a number - purple (within 8 in) and orange (outside 8 in)
  ...
  public void autonomousPeriodic() {
    ...
    if(m_autoSelected == Paths.BOOP1){
      ...
      autonomousBasePID.periodic(-35, 0, 0, 0, 0, 0, 0);
    } else if (m_autoSelected == Paths.SMI){
      autonomousBasePID.periodic(0, 136, 55, 0, 0, 0, 0);
    } else if (m_autoSelected == Paths.SMIO){
      autonomousBasePID.periodic(0, 136, 55, 60, 0, 0, 0); 
  ...
  public void testPeriodic() {
    // climberSubsystem.solenoidOne.set(Value.kReverse);
    // climberSubsystem.solenoidTwo.set(Value.kReverse);
   
    // driveSubsystem.driveTank(0.4, 0.4);
    // driveSubsystem.checkRPM();

    // driveSubsystem.getDegrees();
    // System.out.println("navx ang" + DriveSubsystem.Navx.getAngle());
